...
function updateDockerComposeVersion(version, composePath = path.join(process.cwd(), 'docker-compose.yml')) {
  try {
    ...
    if (compose.services && compose.services['ai-mermaid']) {
      // 獲取現有的 image 名稱（不包含標籤）
      const currentImage = compose.services['ai-mermaid'].image || 'ai-mermaid';
      const imageName = currentImage.includes(':') 
        ? currentImage.split(':')[0] 
        : currentImage;
      
      // 更新版本號，保留原有的 image 名稱前綴
      compose.services['ai-mermaid'].image = `${imageName}:${version}`;

/**
 * @file ai-service.js
 * @description AI服務 - 處理與AI模型的通信和圖表生成
 * 
 * 主要功能：
 * 1. 根據用戶輸入文本生成Mermaid圖表代碼
 * 2. 支持多種圖表類型(流程圖、時序圖、類圖、ER圖等)
 * 3. 處理AI API的請求和響應
 * 4. 支持流式輸出和結果處理
 * 
 * 實現了自動選擇最適合的圖表類型的功能，優先考慮流程圖和時序圖，
...
export async function generateMermaidFromText(text, diagramType = "auto", onChunk = null) {
  let originalType = typeof text;
  if (Array.isArray(text)) {
    text = text.map(t => (typeof t === 'object' ? JSON.stringify(t) : String(t))).join('\n');
  } else if (typeof text === 'object' && text !== null) {
    text = JSON.stringify(text);
  } else {
    text = String(text);
  }
  console.log('[AI Mermaid] generateMermaidFromText input:', text, 'original type:', originalType);
  if (!text) {
  ...
  if (cleanedText.length > parseInt(process.env.NEXT_PUBLIC_MAX_CHARS || "20000")) {
    return { 
      mermaidCode: "", 
      error: `文本超過${process.env.NEXT_PUBLIC_MAX_CHARS || "20000"}字數限制` 
    };
  }
